...
const generateAccessToken = (id, email, roles, isVerified) => {
...
const activateUser = async (req, res) => {
    ...
    try {
        ...
        const { email } = decoded;
        const user = await User.findOneAndUpdate({ email }, { isVerified: true }, { new: true } // Returns the updated document
        );
        if (!user) {
            return res.status(404).json({ error: "User not found" });
        }
        const accessToken = generateAccessToken(user._id, user.email, user.roles, true);
