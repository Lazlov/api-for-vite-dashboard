...
const accessToken = (id, email, roles, isVerified) => {
    const payload = { id, email, roles, isVerified };
    if (secret) {
        return jwt.sign(payload, secret, { expiresIn: "24h" });
    }
};
const passwordToken = (_id) => {
...
const login = async (req, res) => {
    ...
    const generateAccessToken = accessToken(user._id, user.email, user.roles, user.isVerified);
...
const getRefreshToken = async (req, res) => {
    ...
    if (secret) {
        jwt.verify(refreshToken, secret, async (err, decoded) => {
            if (err)
                return res.status(403).json({ err: "error1" });
            const foundUser = await User.findOne({ _id: decoded.id });
            if (!foundUser) {
                return res.status(401).json({ mssg: "user is not logged in" });
            }
            const generateAccessToken = accessToken(foundUser._id, foundUser.email, foundUser.roles, foundUser.isVerified);
